...
def _native_candidates():
    """List native extension filenames to try for this experiment."""
    patterns = [
        os.path.join(_HERE, "yolanda_search_ext.cpython-*.so"),
        os.path.join(_HERE, "yolanda_search_ext.*.so"),
        os.path.join(_HERE, "yolanda_search_ext.so"),
        os.path.join(_HERE, "yolanda_search_ext.cp*.pyd"),
    ]
...
def _import_native_module(path):
    """Load one compiled extension safely and return the imported module."""
    try:
        spec = importlib.util.spec_from_file_location("yolanda_search_ext", path)
        ...
        # If native loading fails, keep the Python fallback alive.
        return None


def _load_native_backend():
    """Try to load Yolanda's optional native backend from this agent folder."""
    for path in _native_candidates():
        mod = _import_native_module(path)
        if mod is not None:
            return mod
    return None


_native_backend = _load_native_backend()
...
def _encode_native_request(b, budget, belief):
    ...
    return (
        ...
        b._carpet_mask,
        b._blocked_mask,
        px, py, pw.points, pw.turns_left,
        ox, oy, ow.points, ow.turns_left,
        float(budget),
        [float(x) for x in belief],
    )


def _decode_native_reply(result):
    """Convert a native reply tuple into a normal Move and score."""
    if result is None:
        return None, None

    mt, direction, roll_length, score = result
    if mt == int(enums.MoveType.SEARCH):
        # Native code packs the search target as (x, y) in the two integer slots.
        mv = Move.search((int(direction), int(roll_length)))
    elif mt == _CARPET_MT:
        mv = Move.carpet(enums.Direction(direction), int(roll_length))
    elif mt == _PRIME_MT:
        mv = Move.prime(enums.Direction(direction))
    else:
        mv = Move.plain(enums.Direction(direction))
    return mv, float(score)


def _run_native_search(b, budget, belief):
    """Call the optional native backend and decode the reply safely."""
    if not _HAS_NATIVE:
        return None, None

    request = _encode_native_request(b, budget, belief)
    try:
        result = _native_backend.search(*request)
    except Exception:
        # A native error should not crash the agent; fall back to Python.
        return None, None
    return _decode_native_reply(result)
...
def _order_moves(moves, tt_hint, killers, history_scores):
    ...
    if len(carpets) > 1:
        carpets.sort(
            key=lambda m: (m.roll_length, history_scores.get(_move_signature(m), 0)),
            reverse=True,
        )

    # For prime/plain moves, use history score as the main ordering signal.
    if len(primes) > 1:
        primes.sort(key=lambda m: history_scores.get(_move_signature(m), 0), reverse=True)
    if len(plains) > 1:
        plains.sort(key=lambda m: history_scores.get(_move_signature(m), 0), reverse=True)
...
class PlayerAgent:
    ...
    def play(self, b: board.Board, sensor_data: Tuple, time_left: Callable):
        ...
        # 4. Search
        if _HAS_NATIVE:
            # Let the native backend try the expensive root search first.
            cpp_move, cpp_score = _run_native_search(b, alloc, self.belief)
            if cpp_move is not None:
                return cpp_move

        best = self._search_iterative(b, search_move, deadline, time_left)
        if best is not None:
            return best
    ...
    # Iterative deepening
    # ------------------------------------------------------------------
    def _search_iterative(self, b, search_move, deadline, time_left):
        ...
        for depth in range(1, MAX_DEPTH + 1):
            ...
            mv, sc = self._search_root_layer(b, cands, search_move, depth, deadline)
            if mv is not None:
                best_move = mv
            else:
    ...
    # Root
    # ------------------------------------------------------------------
    def _search_root_layer(self, b, cands, search_move, depth, deadline):
        ...
        for m in ordered:
            ...
            sc = self._alpha_beta_search(nb, alpha, beta, depth - 1, False, deadline)
            if sc is None:
                return None, None
            if sc > best_sc:
                best_sc = sc
    ...
    # Alpha-Beta
    # ------------------------------------------------------------------
    def _alpha_beta_search(self, b, alpha, beta, depth, maximizing, deadline):
        if time_module.time() >= deadline:
            return None
        ...
        if maximizing:
            ...
            for m in ordered:
                ...
                if use_lmr:
                    ...
                    val = self._alpha_beta_search(nb, alpha, alpha + 1, depth - 2, False, deadline)

                    # If it still looks good, re-search at full depth.
                    if val is not None and val > alpha:
                        val = self._alpha_beta_search(nb, alpha, beta, depth - 1, False, deadline)
                else:
                    val = self._alpha_beta_search(nb, alpha, beta, depth - 1, False, deadline)
            ...
            for m in ordered:
                ...
                if use_lmr:
                    # Cheap reduced-depth scout search first.
                    val = self._alpha_beta_search(nb, beta - 1, beta, depth - 2, True, deadline)

                    # If it still looks dangerous, re-search at full depth.
                    if val is not None and val < beta:
                        val = self._alpha_beta_search(nb, alpha, beta, depth - 1, True, deadline)
                else:
                    val = self._alpha_beta_search(nb, alpha, beta, depth - 1, True, deadline)
    ...
    def _add_killer(self, depth, m):
        key = _move_signature(m)
        ks = self.killer_moves.get(depth)
        if ks is None:
            self.killer_moves[depth] = [key]
            return
        if key not in ks:
            if len(ks) >= 2:
                ks[1] = ks[0]
